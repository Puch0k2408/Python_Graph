...
def draw_axis(x_left, x_right, y_bottom, y_top):
    ...
    while x < x_right:
        if x != 0:
            ...
            if x_canvas != 0:
                canvas.create_line(x_canvas, 0, x_canvas, CANVAS_H, fill='#444', dash=(1, 2))  # Используем dash для пунктирной линии
    ...
    while y >= y_bottom:
        ...
        if y != 0:
            if y_canvas != 0:
                canvas.create_line(0, -y_canvas, CANVAS_W, -y_canvas, fill='#444', dash=(1, 2))  # Используем dash для пунктирной линии
...
def graph_dot(x_tmp, y_tmp, color, line_width=2.5): # Рисование графика
    ...
    for x, y in zip(x_tmp, y_tmp): # Этот цикл выполняется для каждой пары координат x и y из списков x_tmp и y_tmp. Он перебирает точки графика.
        ...
        if prev_x is not None and prev_y is not None:
            # Проверить, пропустить ли текущую точку
            if abs(y - prev_y) < 10: 
                prev_x, prev_y = x_canvas, y_canvas
                continue

            line = canvas.create_line(prev_x, -prev_y, x_canvas, -y_canvas, fill=color, width=line_width) # создается линия между предыдущей и текущей точкой на холсте.
            dot_list.append(line) # Созданная линия добавляется в список dot_list, чтобы ее можно было в последствии удалить или обновить.
...
def initialize_graph():
    ...
    graph4 = graph_dot(x_list, y_list4, 'purple')  # Измените цвет или стиль линии по вашему желанию для четвертого графика
...
def show_help(): # Помощь
    help_text = (
        "Добро пожаловать в программу для построения простейших графиков функций!\n\n"
        ...
        "Для приближения графика используйте кнопку 'Приблизить', а для отдаления - 'Отдалить'.\n\n"
        "Чтобы переместить график, используйте кнопки 'Влево', 'Вправо', 'Вверх' и 'Вниз'.\n\n"
